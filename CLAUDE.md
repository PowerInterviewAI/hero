...
Placeholder imagery (both `public/media/marketing/` and the six pending docs screenshots) is generated by `node scripts/generate-placeholders.mjs`, which is the source of truth for the exact pixel sizes. Each folder's `README.md` says what every placeholder should become.
